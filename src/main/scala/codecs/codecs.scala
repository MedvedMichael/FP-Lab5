...
trait EncoderInstances {
  ...
  implicit val stringEncoder: Encoder[String] = Encoder.fromFunction(n => Json.Str(n))

  /** An encoder for `Boolean` values */
  // TODO Define an implicit value of type `Encoder[Boolean]`
  implicit val boolEncoder: Encoder[Boolean] = Encoder.fromFunction(n => Json.Bool(n))
...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] = Decoder.fromPartialFunction {
    case Json.Num(num) if num.isValidInt => num.toIntExact
  }

  /** A decoder for `String` values */
  // TODO Define an implicit value of type `Decoder[String]`
  implicit val stringDecoder: Decoder[String] = Decoder.fromPartialFunction { case Json.Str(n) => n }

  /** A decoder for `Boolean` values */
  // TODO Define an implicit value of type `Decoder[Boolean]`
  implicit val boolDecoder: Decoder[Boolean] = Decoder.fromPartialFunction { case Json.Bool(n) => n }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(arr) =>
        val res = arr.map(n => decoder.decode(n))
        if (res.forall(i => i.isDefined)) {
          Some(res.map(i => i.get))
        }
        else None
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(map) =>
        ...
        pair match {
          case Some(value) => decoder.decode(value._2)
        }
    }
...
case class Binary(values: List[Boolean], number: Int)

object Binary extends BinaryCodecs

trait BinaryCodecs {

  /** The encoder for `Binary` */
  implicit val binaryEncoder: Encoder[Binary] =
    ObjectEncoder
      .field[List[Boolean]]("values")
      .zip(ObjectEncoder.field[Int]("number"))
      .transform[Binary](binary => (binary.values, binary.number))

  /** The decoder for `Binary` */
  implicit val binaryDecoder: Decoder[Binary] =
    Decoder
      .field[List[Boolean]]("values")
      .zip(Decoder.field[Int]("number"))
      .transform {
        case (values, number) => new Binary(values, number)
      }
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val maybeJsonObj3 = parseJson(""" { "values": [true, false, false, true, true], "number": 5 } """)
    ...
    println(maybeJsonObj3.flatMap(_.decodeAs[Binary]))
    println(renderJson(new Binary(List(true, false, false, true, true), 5)))
  }
